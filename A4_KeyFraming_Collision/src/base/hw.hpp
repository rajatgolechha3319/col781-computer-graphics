...
namespace COL781 {
	namespace OpenGL {
		...
		public:
			...
			void draw_helper(Object &object, int nv, int nt, int ne , const glm::vec3* vertices, const glm::vec3* normals, const glm::ivec3* triangles, const glm::ivec2* edges, AttribBuf &vertexBuf, AttribBuf &normalBuf) {
				vertexBuf = createVertexAttribs(object, 0, nv, vertices);
				normalBuf = createVertexAttribs(object, 1, nv, normals);
				createTriangleIndices(object, nt, triangles);
				createEdgeIndices(object, ne, edges);
			}

			void update_helper(AttribBuf &vertexBuf, AttribBuf &normalBuf, int nv, const glm::vec3* vertices, const glm::vec3* normals) {
				updateVertexAttribs(vertexBuf, nv, vertices);
				updateVertexAttribs(normalBuf, nv, normals);
			}
